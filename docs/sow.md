...
| `assets/pauper/pools.json`        | Trophy leaderboards shared by every profile             |
...
8. **Profile rebuild**: Regenerate the per-player and per-archetype profiles, plus the `assets/pauper/pools.json` trophy leaderboards they rank against
9. **Metagame timeline rebuild**: Regenerate `assets/pauper/meta/timeline.json` with per-event archetype counts
10. **Timestamp**: Write `info.json` with the current UTC datetime
...
- **Trophy leaderboards**: `assets/pauper/pools.json` — one rank-ordered list of names per group (`players`, `decks`) and window (`yearly`, `alltime`). A name's position is its rank and the list's length is the pool size, so neither is copied into the thousands of profiles that display them
